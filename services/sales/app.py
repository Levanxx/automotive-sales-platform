...
def create(h):
    ...
    try:
        with LOCK, managed_connection() as c:
            ps = c.execute('SELECT stage,seller_id FROM prospects WHERE id=?', (d['prospect_id'],)).fetchone()
            ...
            if ps['seller_id'] != d['seller_id']:
                raise APIError(400, 'El vendedor debe coincidir con el asignado al prospecto')
            v = c.execute('SELECT sold FROM vehicles WHERE id=?', (d['vehicle_id'],)).fetchone()
            if v is None: raise APIError(400, 'Vehículo no encontrado')
            if d['status'] == 'completed' and v['sold'] > 0:
                raise APIError(409, 'Este vehículo ya fue vendido')
...
Sales.routes={('GET','/sales'):list_all,('POST','/sales'):create,('GET','/sales/conversion'):conversion,('GET','/sales/conversion/stages'):stage_conversion,('GET','/sales/{id}'):get_one_sale}
